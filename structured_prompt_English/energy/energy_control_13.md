...
The lights can be turned off automatically when not needed, the sockets can be turned off when electricity is not in use, there is no need to turn on the heating when there is no one indoors and the temperature is not very low, there is no need to turn on the air conditioner when opening windows for ventilation can cool down, and close the doors and windows first when you need to turn on the air conditioner.
