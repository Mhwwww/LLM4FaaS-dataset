...
think there is a problem with the logic here. The energy-saving mode should be preset in the system, and then the customer's opinion should be asked when it is actually implemented. For example, there are two examples: 1. I just woke up in the morning and want to ventilate. Then I will tell AI to open the window. At this time, the system should detect the external climate environment by itself, or automatically retrieve today's weather forecast to give corresponding suggestions. For example, it is raining heavily outside now, so it is recommended not to open the window. Whether to turn on the air conditioning ventilation mode. 2. I told AI that it was too hot, and AI also suggested whether I should open the window for ventilation or turn on the air conditioner based on the external environment. Or, I forgot that I turned on the air conditioner and opened the window. Then can AI automatically detect this operation and ask me if I need to turn off the air conditioner? Like the example mentioned in the title, turning on the air conditioner and automatically closing the window, these should be the basic settings set in the default "energy-saving mode". If it is about my expectations for the energy-saving mode settings. There may be several common situations, such as forgetting to turn off the appliance when going out, and automatically raising the temperature by two degrees when falling asleep in summer. I think it is better to ask some questions about which appliances or appliances you hope to be automated and what expectations you hope to achieve. For example, if you have a smoke sensor in your kitchen, why not add one to your range hood as well?
