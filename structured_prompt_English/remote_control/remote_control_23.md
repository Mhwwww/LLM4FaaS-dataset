...
Establish a network control system, with the big system branching out into five small systems (one branch system for each room). Each branch system is further divided into two small systems (sensor system and smart device system), and then set up parallel branches in the sensor, temperature, humidity, and light intensity. Similarly, set up parallel branches in the smart device.
