...
MORNING PLAN:
Turn on the bedroom light, open the curtains, set the music player for 6am, and start the coffee machine to make coffee.

LEAVE HOME PLAN:
Turn off all lights in the house, lock the front door, and close the sockets

MOVIE PLAN:
Close the curtains, dim the lights, turn on the TV and set the program.
