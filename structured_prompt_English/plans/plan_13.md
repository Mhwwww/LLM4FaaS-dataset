...
MORNING PLAN:
The curtains are opened, the lights are dimmed, the coffee machine starts working, and music plays to set the mood.

LEAVE HOME PLAN:
The lights are turned off, the curtains are drawn, the sockets are turned off, and the entrance door is locked.

MOVIE PLAN:
Draw the gauze curtains on the windows, dim the lights, and watch on TV with intelligent controls.
